...
function classifyTestFile(relPath: string, content: string): TestLayer {
	...
	if (/\b(page|browser|navigate|goto|click|fill|expect\(page)\b/.test(content) && /playwright|puppeteer|cypress/.test(content)) return "e2e";
...
function walkTests(dir: string, cwd: string, out: TestFile[]): void {
	for (const entry of readdirSync(dir)) {
		...
		if (!entry.includes(".test.") && !entry.includes(".spec.") && !entry.includes(".e2e.") && !entry.includes(".int.") && !dir.includes("__tests__") && !dir.includes("/e2e") && !dir.includes("/test")) continue;
